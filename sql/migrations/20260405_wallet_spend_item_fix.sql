-- Fix: spend_item case in wallet_apply_mutation was incrementing the inventory
-- before checking if the balance was sufficient.  When the balance was too low
-- (v_reason = 'insufficient_balance') v_should_persist remained false so the DB
-- was never touched, but the wallet JSON returned by the RPC had the wrong
-- inventory values.  Move the inventory increment inside the success branch so
-- the returned snapshot is always consistent with the persisted state.
...
begin
  ...
  case v_action
    ...
    when 'spend_item' then
      -- Resolve item cost first; do NOT touch inventory yet.
      case v_item_key
        when 'joker_fifty_fifty' then v_cost := 25;
        when 'joker_freeze'      then v_cost := 35;
        when 'joker_pass'        then v_cost := 35;
        when 'streak_shield'     then v_cost := 130;
        else v_reason := 'invalid_item';
      end case;
      ...
      if v_reason is null then
        if v_balance < v_cost then
          ...
          -- Only increment inventory after confirming the balance is sufficient.
          case v_item_key
            when 'joker_fifty_fifty' then v_inventory_joker_fifty_fifty := v_inventory_joker_fifty_fifty + 1;
            when 'joker_freeze'      then v_inventory_joker_freeze      := v_inventory_joker_freeze      + 1;
            when 'joker_pass'        then v_inventory_joker_pass        := v_inventory_joker_pass        + 1;
            when 'streak_shield'     then v_inventory_streak_shield     := v_inventory_streak_shield     + 1;
            else null;
          end case;
