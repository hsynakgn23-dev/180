-- Harden public profile reads.
-- Raw profiles rows are now owner-only; public consumers must use the
-- sanitized profiles_public view instead.
...
as $$
  ...
  normalized as (
    select
      ...
      show_activity,
      greatest(
        case when coalesce(xp ->> 'totalXP', '') ~ '^-?\d+$' then greatest((xp ->> 'totalXP')::integer, 0) else 0 end,
        case when coalesce(xp ->> 'xp', '') ~ '^-?\d+$' then greatest((xp ->> 'xp')::integer, 0) else 0 end
      ) as total_xp,
...
select
  ...
  p.display_name,
  greatest(
    case when coalesce(p.xp_state ->> 'totalXP', '') ~ '^-?\d+$' then greatest((p.xp_state ->> 'totalXP')::integer, 0) else 0 end,
    case when coalesce(p.xp_state ->> 'xp', '') ~ '^-?\d+$' then greatest((p.xp_state ->> 'xp')::integer, 0) else 0 end
  ) as total_xp,
