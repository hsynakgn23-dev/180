...
create table if not exists public.wallet_topup_grants (
  ...
  granted_reels integer not null check (granted_reels > 0),
  granted_at timestamptz not null default timezone('utc'::text, now()),
  created_at timestamptz not null default timezone('utc'::text, now())
);

create unique index if not exists wallet_topup_grants_transaction_ref_uidx
  on public.wallet_topup_grants (transaction_ref);

create unique index if not exists wallet_topup_grants_provider_token_hash_uidx
  on public.wallet_topup_grants (provider, purchase_token_hash)
  where purchase_token_hash is not null;

create index if not exists wallet_topup_grants_user_id_idx
  on public.wallet_topup_grants (user_id, granted_at desc);

with existing_topups as (
  select
    profiles.user_id,
    case
      when lower(coalesce(entry.value ->> 'provider', '')) = 'google' then 'google'
      else 'apple'
    end as provider,
    coalesce(entry.value ->> 'ref', '') as transaction_ref,
    coalesce(entry.value ->> 'productId', '') as product_id,
    nullif(entry.value ->> 'purchaseTokenHash', '') as purchase_token_hash,
    nullif(entry.value ->> 'transactionId', '') as transaction_id,
    case
      when coalesce(entry.value ->> 'purchaseDate', '') ~ '^\d{4}-\d{2}-\d{2}T' then (entry.value ->> 'purchaseDate')::timestamptz
      else null
    end as purchase_date,
    nullif(entry.value ->> 'verificationKind', '') as verification_kind,
    case coalesce(entry.value ->> 'productId', '')
      when 'com.absolutecinema.reel.80' then 80
      when 'com.absolutecinema.reel.200' then 200
      when 'com.absolutecinema.reel.550' then 550
      when 'com.absolutecinema.reel.1200' then 1200
      else null
    end as granted_reels,
    coalesce(
      case
        when coalesce(entry.value ->> 'grantedAt', '') ~ '^\d{4}-\d{2}-\d{2}T' then (entry.value ->> 'grantedAt')::timestamptz
        else null
      end,
  ...
  cross join lateral jsonb_array_elements(
    coalesce(profiles.xp_state::jsonb -> 'wallet' -> 'processedTopups', '[]'::jsonb)
  ) as entry(value)
)
insert into public.wallet_topup_grants (
  user_id,
  provider,
  transaction_ref,
  product_id,
...
select
  ...
  granted_at,
  granted_at
from existing_topups
where transaction_ref <> ''
  and granted_reels is not null
...
declare
  ...
  v_purchase_token_hash text := nullif(trim(coalesce(p_purchase_token_hash, '')), '');
  v_transaction_id text := nullif(trim(coalesce(p_transaction_id, '')), '');
  v_purchase_date timestamptz := null;
  v_topup_insert_count integer := 0;
begin
  ...
  begin
    v_purchase_date := nullif(trim(coalesce(p_purchase_date, '')), '')::timestamptz;
  ...
  case v_action
    ...
    when 'grant_topup' then
      ...
      if v_reason is null then
        ...
        values (
          p_user_id,
          case when v_provider = 'google' then 'google' else 'apple' end,
          v_transaction_ref,
          v_product_id,
          v_purchase_token_hash,
          v_transaction_id,
          v_purchase_date,
          coalesce(nullif(trim(coalesce(p_verification_kind, '')), ''), 'store_payload'),
          coalesce(v_reels, 0),
          v_now,
          v_now
        )
        ...
        get diagnostics v_topup_insert_count = row_count;

        if v_topup_insert_count = 0 then
          ...
          v_processed_topups := v_processed_topups || jsonb_build_array(
            jsonb_build_object(
              ...
              'purchaseTokenHash', v_purchase_token_hash,
